...
import { Booking, BookingStatus, PaymentStatus, PayerDetails, Traveler } from '../../types';
                    ...
                    <div className="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
                        ...
                        <div className="flex-1 px-6 py-6 space-y-8 pb-20">
                            ...
                            <div className="flex justify-end space-x-2">
                                {isEditing ? (
                                    <>
                                        <button onClick={handleCancel} className="px-3 py-2 text-sm text-gray-600 bg-gray-100 rounded hover:bg-gray-200">Cancel</button>
                                        <button onClick={handleSave} className="px-3 py-2 text-sm text-white bg-blue-600 rounded hover:bg-blue-700 flex items-center">
                                            <Save className="h-4 w-4 mr-1" /> Save Changes
                                        </button>
                                    </>
                                ) : (
                                    <button onClick={() => setIsEditing(true)} className="px-3 py-2 text-sm text-blue-600 border border-blue-200 bg-blue-50 rounded hover:bg-blue-100 flex items-center">
                                        <Edit2 className="h-4 w-4 mr-1" /> Edit Booking
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className={`p-3 rounded-lg border text-center ${getStatusColor(formData.status)}`}>
                                ...
                                <div>
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className={`p-3 rounded-lg text-center ${getPaymentColor(formData.paymentStatus)}`}>
                            ...
                            <div className="border rounded-xl p-4 bg-gray-50">
                                ...
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between text-green-600">
                                        ...
                                        <span className="font-bold">-{formData.paidAmount.toLocaleString()} SEK</span>
                                    </div>
                                    <div className="border-t border-gray-200 pt-2 flex justify-between font-bold text-gray-900">
                                        <span>Remaining Due</span>
                                        <span>{remainingAmount.toLocaleString()} SEK</span>
                                    </div>
                ...
                <tr key={booking.id} className="hover:bg-gray-50 transition cursor-pointer" onClick={() => setSelectedBooking(booking)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                     <span className={`text-xs font-bold ${
                         booking.paymentStatus === PaymentStatus.PAID ? 'text-green-600' : 
                         booking.paymentStatus === PaymentStatus.PARTIAL ? 'text-orange-600' : 'text-red-600'
                     }`}>
                         {booking.paymentStatus}
                     </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="text-gray-900 font-medium">{booking.paidAmount.toLocaleString()} <span className="text-gray-400 font-normal">/ {booking.totalAmount.toLocaleString()}</span></div>
                    {booking.totalAmount - booking.paidAmount > 0 && (
                        <div className="text-xs text-red-500">Due: {(booking.totalAmount - booking.paidAmount).toLocaleString()}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button className="text-gray-400 hover:text-blue-600" onClick={(e) => { e.stopPropagation(); setSelectedBooking(booking); }}>
